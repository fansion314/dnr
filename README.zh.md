...
| `--window-icon icon.png` | 内嵌可选窗口/Dock 图标，PNG 缩小至最多 128×128。 |
| `--force` | 替换已有输出。 |
...
- **原生库和程序按声明的 group 准备。** v4 包在首次原生加载或执行时准备整个组，优先复用包旁安装内容，其次复用用户持久缓存；普通读取仍走 ZIP。
...
新 dnc 和 dnr 仅支持 v4，采用不压缩的二进制元数据和短原生载荷路径。
v1/v2/v3 包会明确报错，请用原始应用目录和新版 dnc 重新打包。v4 应用及其扩展默认开启 V8 字节码和 TS/JSX
