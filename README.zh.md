...
- **原生库和程序按声明的 group 准备。** v2/v3 包在首次原生加载或执行时准备整个组，优先复用包旁安装内容，其次复用用户持久缓存；普通读取仍走 ZIP。v1 包保持原有临时解压行为。
...
新 dnc 默认输出 v3，采用不压缩的二进制元数据和短原生载荷路径；`--format-version 2`
保留旧格式输出。dnr 继续读取 v1/v2。v3 应用及其扩展默认开启 V8 字节码和 TS/JSX
