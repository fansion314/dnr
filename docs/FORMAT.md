...
应用 ZIP 映射在应用包的真实所在目录，同名文件优先于磁盘内容，目录合并枚举。cwd 保留调用者目录。ZIP 内节点只读，其余写入作用于磁盘；没有 copy-on-write 层或全量解压模式。原生加载器是按需落盘的例外：仅将请求的原生库释放到独立系统临时目录，普通 VFS 读取仍使用 ZIP 内容。外部子进程不会继承系统级挂载。
